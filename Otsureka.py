...
class otsureka:
    ...
    def __init__(self):
        ...
        self.oldImage = None
        self.currentBlackPixels = 0
        self.oldBlackPixels = 0
        self.dailyRGR = 0
        self.hourlyRGR = 0
    ...
    def analyze(self):
        ...
        for i in range(len(self.currentImage)):
            for j in range(len(self.currentImage[0])):
                if self.currentImage[i, j] == 0:
                    self.currentBlackPixels += 1
        for i in range(len(self.oldImage)):
            for j in range(len(self.oldImage[0])):
                if self.oldImage[i, j] == 0:
                    self.oldBlackPixels += 1
        self.pixelDelta = self.currentBlackPixels - self.oldBlackPixels
        self.dailyRGR = np.log(float(self.currentBlackPixels)/self.oldBlackPixels)
        self.hourlyRGR = np.log(float(self.currentBlackPixels)/self.oldBlackPixels)/24
    ...
    def backup(self):
        runner("rclone copy ~/pics/ 'pi1:CPL Lab Group Folder/Cole/Pi6' -v")
    ...
    def main(self):
        ...
        if self.oldImage is None:
            ...
            self.capture()
            self.analyze()
            self.writer.write([self.time, self.currentBlackPixels, self.oldBlackPixels, self.pixelDelta, self.dailyRGR, self.hourlyRGR])
            self.cleanup()
            self.backup()
